...
int CSpectrumInspectionDlg::ReadEvaluationLog(const CString &fullFileName){
	...
	int fileType = flux.ReadSettingFile(fileName, nChannels, fileVersion);
	if(fileType !=1){
		return 1;
	}
	...
	for(int k = 1; k <= nChannels; ++k){
		Flux::CTraverse *tr = new Flux::CTraverse();
		flux.m_traverse.SetAtGrow(flux.m_traverseNum + k, tr);
	}

	// Read the data from the file
	if(0 == flux.ReadLogFile(filePath, fileName, nChannels, fileVersion)){
	...
	for(int k = 0; k <= nChannels; ++k){
		Flux::CTraverse *tr = flux.m_traverse.GetAt(k);
		delete tr;
	}
	flux.m_traverse.RemoveAll();
